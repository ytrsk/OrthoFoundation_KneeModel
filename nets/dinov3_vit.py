...
def get_vit(tag: str, repo_dir: str | None = None, weights: str | None = None):
    ...
    Provide the local DINOv3 torch.hub repository and checkpoint through
    kwargs or environment variables.
    """
    repo_dir = repo_dir or os.environ.get("DINOV3_REPO_DIR")
    weights = weights or os.environ.get("DINOV3_WEIGHTS")
    if repo_dir is None or weights is None:
        raise ValueError(
            "DINOv3 requires repo_dir/weights arguments or DINOV3_REPO_DIR "
            "and DINOV3_WEIGHTS environment variables."
        )
